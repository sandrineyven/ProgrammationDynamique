public class Algo {
	// Paramètres
	private static final int qtot = 20;
	...
	public static void main(String[] args) {
		...
		for (int turbine = 4; turbine > 1; turbine--) {
			...
			for (int xn = 0; xn < sizeSn; xn++) {
				for (int sn = 0; sn < sizeSn; sn++) {
					if (sn - xn < 0) {
						...
						// System.out.println( "sn :" +sn+ " xn : " +xn +" turbine: " + turbine + "
						// Gn[turbine][xn] " + Gn[turbine][xn] + " Fnmax[turbine+1][sn-xn] " +
						// Fnmax[turbine+1][sn-xn]);
					}
			...
			for (int sn = 1; sn < sizeSn; sn++) {
				...
				// System.out.println("turbine: " + turbine +" Fnmax: " + Fnmax[turbine][sn] + "
				// qn: " + Qn[turbine][sn]);
			}
			for (int sn = 0; sn < sizeSn; sn++) {
		...
		for (int turbine = 1; turbine < 6; turbine++) {
			for (int xn = 1; xn < sizeSn; xn++) {
				//System.out.println("turbine: 1  FnMax: " + Fnmax[turbine][xn] + "turbine: 1  QnMax: " + Qn[turbine][xn]);
			}
		...
		System.out.println(Qn[1][sizeSn - 1]);
		int Q1 = Qn[1][sizeSn - 1];
		double P1 = Fnmax[1][sizeSn - 1];
		System.out.println("q1 = "  + Q1 + " p1 = " + P1);
		int Q2 = qtot - Qn[2][Q1/5];
		double P2 = Fnmax[2][Q1/5];
		System.out.println("q2 = " + Q2+ " p2 = " + P2);
		
		//System.out.println(Qn[3][Q2/5]);
	...
	private static double[][] initGn(int sizeSn, int[] Sn) {
		...
		for (int turbine = 1; turbine <= 5; turbine++) {
			for (int sn = 0; sn < sizeSn; sn++) {
				...
				Gn[turbine][sn] = p;
				// System.out.println("turbine " + turbine + " Gn[turbine][sn]: " +
				// Gn[turbine][sn]);
			}
		}
		return Gn;
